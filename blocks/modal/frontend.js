...
(function($){
	...
	var initElements = function(){
		...
		$('.baytek-block-popup').each(function(){
			// for the identified modal we add an instance of the class BaytekModal 
			// to the data element  
			$(this).data('baytek-block-popup', new BaytekModal(this));
	...
	var BaytekModal = function (container) {
		...
		this.$container.find('.baytek-modal-content').attr('id' , 'baytekModalAriaContent' + window.baytekModalCount);
		...
		this.$overlay = this.$container.find('.baytek-block-popup-overlay')
			.on('click' , this.hide);
		this.$modalWrap = this.$container.find('.baytek-block-popup-wrap');
		this.$modalcloser = this.$container.find('.baytek-block-popup-closer')
		.on('click' , this.hide);
	...
	BaytekModal.prototype = {
		show: function(){

			// start clearing any timeouts
			...
			// invisible. We wait to ensure the modal is ready then add
			// an active class which triggers the opacity transition
